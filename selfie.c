...
int  getContextUser(int* context)  { return        *(context + 11); }
int  getContextOS(int* context)  { return        *(context + 12); }
...
void setContextUser(int* context, int userProcess)   { *(context + 11) = userProcess; }
void setContextOS(int* context, int os)   { *(context + 12) = os; }
...
int* allocateContext(int ID, int parentID) {
  ...
  if (freeContexts == (int*) 0)
    context = malloc(7 * SIZEOFINTSTAR + 6 * SIZEOFINT);
  else {
  ...
  setParent(context, parentID);
	...
	setContextUser(context,userProcess);
	setContextOS(context,exceptionHandlingOS);
...
int runOrHostUntilExitWithPageFaultHandling(int toID) {
  ...
  while (1) {
		...
		printd("userProcess",userProcess);
		printd("Exception handlin",exceptionHandlingOS);	
		printd("HAs User Process",hasUserProcess(usedContexts));
		println();

    fromID = selfie_switch(toID);

		isUserProcess = getContextUser(currentContext);
    ...
    if (getParent(fromContext) != selfie_ID()){
      ...
      if (exceptionNumber == EXCEPTION_PAGEFAULT) {
        ...
        return -1;
			...
			// FIXME iterate over context
			if(userProcess == 0){
				if(hasUserProcess(usedContexts) == 1){
					if(exceptionHandlingOS == 0){
